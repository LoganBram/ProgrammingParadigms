...
spiral_seq n = if n < 1 then "" else spiral_seq (n - 1) ++ ";" ++ show (spiral n 260)
...
{-
  ...
  => incr 9                               by if-then-else rule and function application substituting 9 for x

  => 9 + 1                                by function application and substituting 9 for x
...
Q4.3:
  ...
  => \y -> (0, incr)                      Function application with incr for x
-}
