...
rewrite important list
  | null list = []
  | important (head list) = head list : head list : rewrite important (tail list)
  | otherwise = head list : rewrite important (tail list)
...
test_genCompare1 =
  genCompare (\i -> \j -> i < j) [1, 2, 4] [3, 2, 0]
    == [True, False, False]
...
When combining two lists, zipWith will stop when it reaches
the end of the shorter list. This is because compares as many elements
as are in the shorter list, whereas listCompare will compare all elements
by adding a false on the end of the list for every element more that a list has.
...
sing (Harmony (Harmony (Atom x) z) _)
  | 'k' `elem` x = z
sing (Harmony (Harmony (Harmony (Atom a) b) c) d)
  | 's' `elem` a = Harmony (Harmony b d) (Harmony c d)
sing (Harmony x y) = Harmony (sing x) (sing y)
sing other = other -- (you're allowed to change this line if you need to)

ascend = Harmony (Harmony (Atom "s0") (Atom "k1")) (Atom "k2")

test_sing1 =
  (sing (Harmony ascend (Atom "k3")))
    == Harmony
      (Harmony (Atom "k1") (Atom "k3"))
...
test_sing4 =
  sing (Harmony (Atom "s?") (Harmony ascend (Atom "k0")))
    == Harmony
      (Atom "s?")
      ( Harmony
          (Harmony (Atom "k1") (Atom "k0"))
